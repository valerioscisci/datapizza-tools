...
│       ├── api/database/     # SQLAlchemy models (Job, User, Application, News, Course, Experience, Education, Proposal, ProposalCourse, ProposalMilestone, ProposalMessage, AICache), connection, seed
│       ├── api/services/     # Business logic services (GeminiAdvisor — AI-powered job matching and career recommendations)
...
│       └── tests/            # pytest unit test suite (333 tests)
...
- **Swagger / OpenAPI Documentation** — Full Swagger UI at `/docs` with custom OpenAPI schema. 13 tag groups, JWT Bearer security scheme, per-endpoint summaries and error responses, Field descriptions on all Pydantic schemas. Public endpoints marked with `openapi_extra={"security": []}`. Centralized tag metadata and custom schema generator in `api/openapi.py`
- **Backend unit test suite** — 333 pytest tests covering auth, routes (auth, jobs, news, courses, profile, applications, talents, proposals, messages, ai), schemas, services, and utilities with full mocking (no real DB). All passing
