...
- Profile page integration with quick notification preferences section
...
│       ├── api/services/     # Business logic services (GeminiAdvisor — AI-powered job matching and career recommendations, EmailService — local email notifications)
...
- **Backend unit test suite** — 377 pytest tests covering auth, routes (auth, jobs, news, courses, profile, applications, talents, proposals, messages, ai, notifications), schemas, services (GeminiAdvisor, EmailService), and utilities with full mocking (no real DB). All passing
