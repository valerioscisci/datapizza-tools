...
- **Automated Daily Digest (Backend + GitHub Action)** — New bulk digest endpoint `POST /api/v1/notifications/daily-digest/bulk` authenticated via `X-API-Key` header (not JWT, system-level cron). Iterates all users with `daily_digest=True` (including those with no preference record, which defaults to enabled), calls `EmailService.generate_daily_digest()` for each with per-user error isolation (one failure doesn't block others). Returns summary report (sent/skipped/failed/total/errors). GitHub Action `daily-digest.yml` (disabled, manual dispatch via `workflow_dispatch`) triggers the endpoint at 9 AM Italian time (cron `0 7 * * *` UTC = 9:00 CEST). Requires `CRON_API_KEY` in both Vercel env vars and GitHub Secrets
- **AI Skills Gap Analyzer (Backend)** — Hybrid algorithmic + Gemini AI skill gap analysis. 3 new endpoints: POST/GET /ai/skill-gap-analysis (per-user, 24h cache) with color-coded demand status (green/yellow/red), missing skills detection with course recommendations, and AI-generated personalized insights in Italian; GET /ai/market-trends (global, 1h cache) with trend direction and change percentages. Graceful degradation: returns algorithmic data even when Gemini is unavailable (no 503). New GeminiAdvisor.skill_gap_analysis() method with dedicated Italian prompt template. 5 new Pydantic schemas with Literal type validation (SkillDemandStatus, MissingSkill, MarketTrend, SkillGapAnalysisResponse, MarketTrendsResponse). Case-insensitive skill matching between user profiles and job tags
- **AI Skills Gap Analyzer (Frontend)** — Interactive dashboard page (`/it/skill-gap`) with 4-section 2-column grid layout: user skills table with color-coded status dots (green=Richiesta, yellow=Stabile, red=In calo) and trend arrows with percentages; missing skills cards with demand level badges, AI-generated personalized reasons, and linked course recommendations from the catalog; ranked market trends list with proportional bar indicators; AI personalized insights panel with multi-paragraph Italian career advice. Auth guard for talent users only (company users redirected to /it/azienda/proposte). Navbar "Skill Gap" link in desktop dropdown, right-side links, and mobile menu (talent-only). Profile page CTA card linking to the dashboard. Loading skeletons, error/retry states, no-skills warning banner with profile link, AI unavailable info banner. Feature-scoped directory structure with 5 components, 4 .props.ts files, 1 custom hook, TypeScript types, and ~45 i18n keys in dedicated skill-gap.json translation file
- **Backend unit test suite** — 465 pytest tests covering auth, routes (auth, jobs, news, courses, profile, applications, talents, proposals, messages, ai, ai-skill-gap, notifications), schemas, services (GeminiAdvisor, EmailService, TelegramService), and utilities with full mocking (no real DB). All passing
