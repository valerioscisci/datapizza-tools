...
- **AI Skills Gap Analyzer** (`plans/02-ai-skills-gap-dashboard.md`) — Interactive dashboard (`/it/skill-gap`) showing user skills vs market demand with color-coded status (green=in demand, yellow=stable, red=declining), missing skills detection with course recommendations, market trend monitoring from Italian job listings, and Gemini-powered personalized insights
...
- **Backend unit test suite** — 424 pytest tests covering auth, routes (auth, jobs, news, courses, profile, applications, talents, proposals, messages, ai, notifications), schemas, services (GeminiAdvisor, EmailService, TelegramService), and utilities with full mocking (no real DB). All passing
